...
// the contact name resolved from the cache.
func (d *DB) Conversations() ([]Conversation, error) {
	rows, err := d.db.Query(`
		SELECT m.address,
		...
		FROM messages m
		JOIN (
			SELECT address, MAX(date) AS maxdate, COUNT(*) AS n
			FROM messages GROUP BY address
		) t ON t.address = m.address AND t.maxdate = m.date
	...
	for rows.Next() {
		var c Conversation
		if err := rows.Scan(&c.Address, &c.ContactName, &c.LastBody, &c.LastDate, &c.Count); err != nil {
			return nil, err
		}
		out = append(out, c)
	}
...
// Messages returns all messages for an address in chronological order.
func (d *DB) Messages(address string) ([]StoredMessage, error) {
	rows, err := d.db.Query(`
		SELECT android_id, address, body, date, type
		FROM messages WHERE address = ? ORDER BY date ASC, android_id ASC`, address)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
